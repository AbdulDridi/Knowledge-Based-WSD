import nltk
import sys
sys.path.insert(0, '../PPR')
from PPRPythonLexicalDisambiguator import *

#load all data needed into the relevant data structures
corpus1 = nltk.corpus.reader.semcor.SemcorCorpusReader(".", "d00.xml",
                                                                   nltk.wordnet)
corpus2 = nltk.corpus.reader.semcor.SemcorCorpusReader(".", "d01.xml",
                                                                   nltk.wordnet)
corpus3 = nltk.corpus.reader.semcor.SemcorCorpusReader(".", "d02.xml",
                                                                   nltk.wordnet)
...
for s in range(len(sentences)):
    setOfSynsets = getSetSynsets(sentences[s])
    sentencesSynsets.append(setOfSynsets[0])
    sentencesMeaningfulSynsets.append(setOfSynsets[1])
    #SECTION THAT DOES DISAMBIGUATION
    disambiguatedSentence = performPPRAndGetSynsetsFromSynsets(
        sentencesMeaningfulSynsets[s], transitionMatrix,synsetIndexes, 0.85, 30)
...
taggedSentences = list(corpus1.tagged_sents("d00.xml","both"))
for s in corpus2.tagged_sents("d01.xml","both"):
    taggedSentences.append(s)
for s in corpus3.tagged_sents("d02.xml","both"):
    taggedSentences.append(s)
